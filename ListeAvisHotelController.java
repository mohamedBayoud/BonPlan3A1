...
public class ListeAvisHotelController implements Initializable {
	...
	private void Modifier(ActionEvent event) throws IOException, SQLException {
		{
			...
			if ((NoteService >= 0 && NoteService <= 5) && (NoteRapport >= 0 && NoteRapport <= 5) && (NoteConfort >= 0 && NoteConfort <= 5) && (NotePersonnel >= 0 && NotePersonnel <= 5)) 
			{
	...
	private void ButtonStat(ActionEvent event) throws IOException {
		{
		 FXMLLoader loader = new FXMLLoader(getClass().getResource("StatisticsHotel.fxml"));
		    Parent root = loader.load();
            StatisticsHotelController frc = loader.getController();
           frc.setRole(role.getText());
           frc.setEmail(email_client.getText());
	       String e = id_client.getText();
           int r = Integer.parseInt(e);
